...
export class FileUploadService {
  ...
  pushFileToStorageforGalarya2(fileUpload: FileUpload,form): Observable<number | undefined> {
    let time = Date.now();
    const filePath = `${this.basePath}/${time}${fileUpload.file.name}`;
    const storageRef = this.storage.ref(filePath);
    const uploadTask = this.storage.upload(filePath, fileUpload.file);
    ...
    uploadTask.snapshotChanges().pipe(
      finalize(() => {
        storageRef.getDownloadURL().subscribe(downloadURL => {
          ...
          fileUpload.url = downloadURL;
          fileUpload.name = fileUpload.file.name;
          this.saveFileDataforfamily2(fileUpload,form);
        });
      })
    ).subscribe();
  ...
  private saveFileDataforfamily(fileUpload: FileUpload,family:any,name:any): void {
   ...
   private saveFileDataforfamily2(fileUpload: FileUpload,data:any): void {
    console.log(fileUpload);
    let obj = {
      url: fileUpload.url,
      family:data.family,
